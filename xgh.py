...
def parse_version(v):
    ...
    if not m:
        print "ERROR: Cannot parse version string '%s'" % v
    else:
...
def get_xpd_xml(path=None):
    ...
    try:
        ...
        print "ERROR: Failed to open xpd.xml"
        sys.exit(1)
...
def get_master_xpd_xml(path=None):
    ...
    if not err_lines:
        ...
        print "ERROR: finding version of xpd.xml on HEAD of master branch"
        print err_lines
        sys.exit(1)
...
def get_dependencies(path=None):
    ...
    deps = []
    xpd_nodes = dom.getElementsByTagName('xpd')
    for xpd_node in xpd_nodes:
        for node in xpd_node.childNodes:
            if node.localName != 'dependency':
                continue
...
def getdeps():
    for dep in get_dependencies():
        pathname = os.path.join("..", dep['name'])
        if os.path.exists(pathname):
            print "Dependency %s already exists" % pathname
...
def checkout(args):
    ...
    print "'%s' not a known version" % version
